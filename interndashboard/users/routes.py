...
def signup():
    ...
    if request.method == 'POST':
        ...
        if password == confirmpassword:
            user = User(studentnumber=studentnumber, firstname=firstname, surname=surname,
                        major=major, degree=degree, studentmail=studentmail, password=password)
            db.session.add(user)
            db.session.commit()
...
def dashboard(studentnumber):
    user = db.session.execute(db.select(User).filter_by(
        studentnumber=session['studentnumber'])).scalar()

    # Dashboard of approved topics
    outputs = Dashboard.query.filter_by(
        studentnumber=session['studentnumber']).all()

    # Dashboard that has to be approved
    topics_model = Topic_create.query.filter_by(
