...
@adminbp.route('/studentsdashboard')
@login_required
...
@adminbp.route('/approvedisapprove/<studentnumber>')
@login_required
def approvedisapprove(studentnumber):
    if session['studentnumber'] == 1:
        ...
        results = Topic_create.query.filter_by(
            studentnumber=studentnumber).all()

        disaprove = appr_disappr.query.filter_by(
            studentnumber=studentnumber).all()
        list_disaprove = []
        for l in disaprove:
        ...
        datas_dict = {"id": [], "topics": [], "description": [],
        ...
        if outputs:
            ...
            print()
            print(df)
            fig1 = px.timeline(df, x_start='startingdate', x_end='finishdate',
                               y='id', color='percentages', hover_name='topics')
...
@adminbp.route('/approve/<id>/<studentnumber>/<topics>')
@login_required
def approve(studentnumber, topics, id):
    if session['studentnumber'] == 1:
        topic = Topic_create.query.filter_by(id=id).first_or_404()
        dash = Dashboard(studentnumber=topic.studentnumber, topics=topic.topics,
                         description=topic.description, startingdate=topic.startingdate, finishdate=topic.finishdate)
        db.session.add(dash)
        ...
        return redirect(url_for('adminbp.approvedisapprove', studentnumber=studentnumber))
    else:
        flash('Access denied...', 'danger')
        return redirect(request.url)


@adminbp.route('/disapprove/<id>/<studentnumber>/<topics>')
@login_required
def disapprove(studentnumber, topics, id):
    if session['studentnumber'] == 1:
        topic = Topic_create.query.filter_by(id=id).first_or_404()
        disapprove = appr_disappr(id_topic=topic.id, studentnumber=topic.studentnumber, topics=topic.topics,
                                  description=topic.description, startingdate=topic.startingdate, finishdate=topic.finishdate)
